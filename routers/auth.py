...
async def create_standard_user(username: Annotated[str, Form()], 
    ...
    if not domain_valid:
        raise HTTPException(status_code=400, detail="Bad request: Invalid Email Address")
    
    if password != repassword:
        raise HTTPException(status_code=400, detail="Bad request: passowords do not match!")
